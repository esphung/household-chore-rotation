//
//  Household_Chore_RotationTests.swift
...
struct Household_Chore_RotationTests {
	...
	@Test func nextDueAtIsComputedFromSchedule() {
		...
		let chore = store.allChores[0]
		guard let nextDueAt = chore.nextDueAt, let lastCompletedAt = chore.lastCompletedAt else {
	...
	@Test func expiredChoreIsReportedAsPending() {
		...
		let chore = Chore(title: "Dishes", schedule: .daily, lastCompletedAt: pastDate, nextDueAt: pastDate)
	...
	@Test func nonExpiredChoreRemainsCompleted() {
		...
		let chore = Chore(title: "Dishes", schedule: .daily, lastCompletedAt: Date(), nextDueAt: futureDate)
		let store = ChoreStore(chores: [chore], completedIndices: [0])
	...
	@Test func checkAndResetOnlyResetsExpiredChores() {
		let pastDate = Date(timeIntervalSinceNow: -1)
		let futureDate = Date(timeIntervalSinceNow: 86400)
		let expiredChore = Chore(title: "Dishes", schedule: .daily, lastCompletedAt: pastDate, nextDueAt: pastDate)
		let activeChore = Chore(title: "Vacuum", schedule: .weekly, lastCompletedAt: Date(), nextDueAt: futureDate)
		let store = ChoreStore(chores: [expiredChore, activeChore], completedIndices: [0, 1])
