...
final class ChoreStore {
		private enum PersistedStateCodingKeys: String, CodingKey {
				case chores
				case currentIndex
		...
		private struct PersistedState: Codable {
				...
				init(from decoder: Decoder) throws {
						let container = try decoder.container(
								keyedBy: PersistedStateCodingKeys.self)
						chores = try container.decode([Chore].self, forKey: .chores)
						currentIndex = try container.decode(Int.self, forKey: .currentIndex)
						completedIndices = try container.decode(
								[Int].self, forKey: .completedIndices)
						selectedSchedule =
								try container.decodeIfPresent(
										ChoreSchedule.self, forKey: .selectedSchedule)
								?? .daily
				}
		}
...
extension ChoreStore {
		...
		func toggleCurrentChoreCompletion() {
				...
				if completedIndices.contains(currentIndex) {
						...
						chores[currentIndex].nextDueAt = chores[currentIndex].schedule.nextDueDate(
								from: now)
				}
				persistState()
		}

		func setCurrentChore(at index: Int) {
				guard chores.indices.contains(index), index != currentIndex else { return }
				currentIndex = index
				persistState()
		}
		...
		func addChores(_ newChores: [Chore]) {
				var didAddAny = false
				for chore in newChores where addChoreInternal(chore) {
						didAddAny = true
				}
		...
		func removeChores(atOffsets offsets: IndexSet) {
				...
				if chores.isEmpty {
						...
						currentIndex = min(
								max(oldCurrentIndex - removedBeforeCurrent, 0), chores.count - 1)
				}
