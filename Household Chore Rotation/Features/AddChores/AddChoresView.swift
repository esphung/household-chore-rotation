		...
		GradientScrollScreen {
			...
			VStack(alignment: .leading, spacing: 16) {
				sectionCard(title: "Add a Chore") {
					...
					Picker("Schedule", selection: $choreStore.selectedSchedule) {
				...
				sectionCard(title: "Current Chores") {
					if choreStore.allChores.isEmpty {
						ThemedText("No chores yet", type: .body, style: .secondary)
					} else {
						ForEach(Array(choreStore.allChores.enumerated()), id: \.offset) {
							index, chore in
							HStack(spacing: 12) {
								ThemedText("\(index + 1).", type: .body, style: .secondary)
								...
								ThemedText(
									...
									style: .custom(AnyShapeStyle(chore.schedule.badgeColor))
								)
								.badgeStyle(
								...
								Button {
									choreStore.removeChores(atOffsets: IndexSet(integer: index))
								} label: {
	...
	NavigationStack {
		AddChoresView(
			choreStore: ChoreStore(chores: [
				Chore(title: "Dishes", schedule: .daily), Chore(title: "Vacuum", schedule: .weekly),
			]))
	}
}
